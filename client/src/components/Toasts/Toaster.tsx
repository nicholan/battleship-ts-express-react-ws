...
import type { GameInviteToastProps } from "./GameInviteToast";
import type { RematchToastProps } from "./RematchToast";
...
const toasts: Record<ToastKeys, (props?: ToastDispatchProps) => void> = {
	...
	PLAYER_JOIN: (props) => {
		props &&
			...
			toast(
				<p>
					<span className="text-orange-400">{props.name}</span> has joined the
					game!
				</p>,
			);
	},
