...
export function Layout() {
	...
	const icon =
		colorScheme === "dark" ? (
			<IconSunFilled size={24} tabIndex={-1} />
		) : (
			<IconMoonFilled size={24} tabIndex={-1} />
		);
		...
		>
			...
			>
				...
				>
					<Tooltip
						position="bottom"
						tooltipText={colorScheme === "dark" ? "Light" : "Dark"}
					>
						<button
							role="button"
							type="button"
			...
			<ToastContainer
				...
				toastClassName={
					"bg-gradient-to-b from-neutral-800 to-neutral-900 border dark:border-neutral-300/10"
				}
				bodyClassName={
					"text-lg lg:text-xl tracking-wider font-staatliches text-white text-center"
				}
			/>
		</div>
	);
