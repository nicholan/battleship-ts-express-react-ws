...
test.describe('Form', () => {
	...
	test('has game code field', async ({ page }) => {
		await expect(page.getByRole('textbox', { name: 'Join existing game with a code' })).toBeVisible();
	});
	...
	test('has no public name element when name field is empty', async ({ page }) => {
