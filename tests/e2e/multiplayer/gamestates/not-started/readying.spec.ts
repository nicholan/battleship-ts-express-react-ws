...
test.describe("Multiplayer", () => {
	test("waiting for other player is correct", async ({
		playerOne,
		playerTwo,
	}) => {
		...
		await expect(
			playerOne.page.getByRole("alert").filter({ hasText: "Game started" }),
		).not.toBeVisible();
	});

	test("game started toast is visible when both players ready up", async ({
		playerOne,
		playerTwo,
	}) => {
		const { gameCode, p1, p2 } = await createMultiplayer(2);
		...
		await expect(
			playerTwo.page.getByRole("alert").filter({ hasText: "Game started" }),
		).toBeVisible();
	});
});
